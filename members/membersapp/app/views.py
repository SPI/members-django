...
def update_member_activity(request, user):
    ...
    if user.downgraded_user:
        user.iscontrib = True
        messages.success(request, 'Retrieved contributing member status! Please wait for up to 5 minutes for changes to propagate to mailing list sites')
    user.save()
...
def voteresult(request, ref):
    ...
    for ballot in ballots:
        ...
        if vote.nb_contrib is not None:
            ballot.vote_percent = round(len(membervotes) / vote.nb_contrib * 100, 2)
            if ballot.quorum is not None:
